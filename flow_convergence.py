...
Run
---
    python flow_convergence.py                     # auto-find output/
    python flow_convergence.py --dir output --plots
    python flow_convergence.py --lb-min 0.9 --T 0  # focus the high-lb band
    python flow_convergence.py --N-min 256000      # mirror "start high" protocol
...
def value_extrap(Ns, ys, sigma):
    """Extrapolate plateau value to N->inf via y = d_inf + a*N^-b.

    Mirrors size_extrapolation: try a 3-param weighted fit; if it is
    ill-conditioned or there are too few points, fall back to a fixed-b
    (b = 2/d_guess) 2-param linear fit. With 2 points, report only the
    delta + direction; with 1, the lone value.
    """
    Ns = np.asarray(Ns, float)
    ys = np.asarray(ys, float)
    sigma = np.asarray(sigma, float)
    med = np.nanmedian(sigma[np.isfinite(sigma) & (sigma > 0)]) if np.isfinite(sigma).any() else 1.0
    if not np.isfinite(med) or med <= 0:
        med = 1.0
    sigma = np.where(np.isfinite(sigma) & (sigma > 0), sigma, med)
    n = len(Ns)
    if n == 1:
        return dict(d_inf=float(ys[0]), ci=float("nan"), beta=float("nan"), model="1N")
    if n == 2:
    ...
    if _HAVE_SCIPY:
        try:
            ...
            lo = [min(ys) - 2.0, -50.0, 0.05]
            hi = [max(ys) + 2.0, 50.0, 3.0]
            popt, pcov = curve_fit(_powlaw, Ns, ys, p0=p0, sigma=sigma,
                                   absolute_sigma=True, maxfev=20000,
                                   bounds=(lo, hi))
            c, a, b = popt
            ci = 1.96 * math.sqrt(max(pcov[0, 0], 0.0))
            if (np.isfinite(ci) and ci < 0.5 * max(abs(c), 1e-9) + 0.5
                    and 0.05 < b < 3.0):
                return dict(d_inf=float(c), ci=float(ci), beta=float(b),
                            model="3param")
        except Exception:
            pass
    # fixed-beta fallback
    yg = ys[-1] if (np.isfinite(ys[-1]) and ys[-1] > 0.5) else 4.0
    b = 2.0 / max(yg, 1.0)
    x = np.power(Ns, -b)
    w = 1.0 / sigma
    try:
        coef, cov = np.polyfit(x, ys, 1, w=w, cov=True)
        c = float(coef[1])
        ci = 1.96 * math.sqrt(max(cov[1, 1], 0.0))
    except Exception:
        c = float(np.average(ys, weights=w))
        ci = float("nan")
    return dict(d_inf=c, ci=ci, beta=float(b), model="fixedb")
...
def write_csv(cells, path):
    fields = ["k", "T", "lb", "n_N", "N_max", "score", "verdict",
